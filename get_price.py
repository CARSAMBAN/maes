...
@app.route('/fiyatcek', methods=['GET'])
def fiyat_cek():
    try:
        # get_price.py dosyasını terminalde çalıştır
        subprocess.run(["python", "get_price.py"], check=True)
        return jsonify({"status": "ok", "message": "Fiyat çekme işlemi tamamlandı ✅"})
    except subprocess.CalledProcessError as e:
        return jsonify({"status": "error", "message": f"Hata: {str(e)}"})
    except Exception as e:
        return jsonify({"status": "error", "message": f"Beklenmeyen hata: {str(e)}"})
...
url = "https://www.vodafone.com.tr/faturaya-ek/samsung-galaxy-a07/p/telefonlar"
driver.get(url)
time.sleep(10)
